...
```
_Figure 13.1 - Operator Overloading in Action_
...
#templates #functiontemplates #classtemplates #genericprogramming

Templates enable writing **generic and reusable code** that works with multiple data types.  
- They eliminate redundancy and increase flexibility in both **functions** and **classes**.

### Function Templates

#templates #functiontemplates

A **function template** defines a single blueprint for a set of related functions.  
Instead of rewriting similar functions for each type, use a type parameter.
...
```
_Figure 13.26 - `template` Function Implementation Example_
- `typename`  introduces a **type parameter**.
- Compiler automatically generates specific versions (instantiations) when called.

```cpp
cout << add(3, 4);      // int version  
cout << add(3.5, 4.5);  // double version
```
_Figure 13.27 - `template` Function Usage Example_

### Class Templates

#templates #classtemplates

A **class template** provides a general definition for a family of classes.  
- Each type-specific version is instantiated automatically when used.
...
```
_Figure 13.28 - `template` Class Implementation Example_
- `class`  introduces a **type parameter**.
- Compiler automatically generates specific versions (instantiations) when called.
...
```
_Figure 13.29 - `template` Class Usage Example_